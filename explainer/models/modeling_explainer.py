...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            # Handle different rope_scaling configurations
            if not isinstance(self.config.rope_scaling, dict):
                # If rope_scaling is not a dict, treat it as None and use default
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
            ...
            if "type" not in self.config.rope_scaling:
                # If type key is missing, use default
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
            ...
            if scaling_type == "linear":
                ...
                # Handle Llama 3.x specific rope scaling
                ...
                original_max_position_embeddings = self.config.rope_scaling.get("original_max_position_embeddings", 8192)
                
                # For Llama 3.x, use regular rotary embedding but with adjusted parameters
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
                ...
                # For unknown scaling types, fall back to default
                print(f"Warning: Unknown RoPE scaling type {scaling_type}, using default rotary embedding")
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
